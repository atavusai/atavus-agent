...
echo "→ Windows 386..."
CGO_ENABLED=0 GOOS=windows GOARCH=386 /usr/local/go/bin/go build \
  -ldflags="-s -w -X main.platform=windows -X main.version=${VERSION}" \
...
# ── macOS amd64 (Intel) ──
echo "→ macOS amd64..."
CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 /usr/local/go/bin/go build \
...
echo "→ macOS arm64..."
CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 /usr/local/go/bin/go build \
  -ldflags="-s -w -X main.platform=macos -X main.version=${VERSION}" \
